...
class DatabaseManager:
    ...
    def save_lspr_ai_prediction(
        ...
        try:
            cursor = self.conn.cursor()
            timestamp = time.strftime('%Y-%m-%d %H:%M:%S')
            normalized_input_context = dict(input_context or {})
            normalized_input_context.setdefault("requested_prediction_model", normalized_input_context.get("model_mode", "auto"))
            normalized_input_context.setdefault("resolved_prediction_model", normalized_input_context.get("model_mode", "auto"))
            normalized_input_context.setdefault("requested_generator_model", normalized_input_context.get("model_mode", "auto"))
            normalized_input_context.setdefault("resolved_generator_model", normalized_input_context.get("model_mode", "auto"))
            normalized_input_context.setdefault("fallback_applied", False)
            normalized_input_context.setdefault("fallback_reason", None)
            payload = json.dumps(normalized_input_context, ensure_ascii=False)
