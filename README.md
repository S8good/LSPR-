...
- `lspr_default_prediction_model`
- `lspr_default_generator_model`
...
- `LSPR AI Workbench` 当前采用“双模型职责拆分”策略：`Prediction Model` 负责浓度预测，`Spectrum Generator` 负责谱线生成与数字孪生 AI 叠加
- 默认情况下，浓度预测优先使用 `v2`；部分 fusion / stage3 模式若浓度推理塌零，会回退到 `v2`
