...
3. 选择 `Backend`、`Prediction Model` 和 `Spectrum Generator`。
...
- `Prediction Model` 负责浓度预测
- `Spectrum Generator` 负责生成谱、对齐谱和数字孪生中的 AI 叠加谱
- 某些 fusion / stage3 模式在浓度推理不稳定时，会自动回退到 `v2`，界面会明确显示
...
- `requested / resolved` 的 prediction model 与 generator model
- fallback 状态与原因
...
- `v2` 主要作为稳定的浓度预测器
- 带 generator 的模式主要承担谱线生成与数字孪生解释职责
